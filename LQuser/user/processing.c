...
//  }
  ...
  if(isEdgeLeft[LINE] && !isEdgeRight[LINE]){
    graph[LINE][LimitingAmplitude(edgeLeft[LINE] + 29, 0, 93)] = 0;
    return LimitingAmplitude(edgeLeft[LINE] + 29, 0, 93);
  }else if(!isEdgeLeft[LINE] && isEdgeRight[LINE]){
    graph[LINE][LimitingAmplitude(edgeRight[LINE] - 29, 0, 93)] = 0;
    return LimitingAmplitude(edgeRight[LINE] - 29, 0, 93);
  }
  ...
  if(IsStaightLine(edgeRight, isEdgeRight, 45, 35, 2, 0.5)){
    for(int i = LINE_TERMINATION; i < LINE_INITIAL - 7; i++){
      if(isEdgeLeft[i] && isEdgeLeft[i+1] && isEdgeLeft[i+2] && !isEdgeLeft[i+3] && !isEdgeLeft[i+4] && !isEdgeLeft[i+5]){
        if(edgeLeft[i+2] < 7 || !isEdgeRight[i+2] || !isEdgeRight[i+3] || !isEdgeRight[i+5] || !isEdgeRight[i+6]){
          continue;
        }
