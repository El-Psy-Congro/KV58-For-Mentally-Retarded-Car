...
bool IsDisconnectRoad(){
  if(GraphProcessingOfLineWhitePointCounting(GRAPH_HIGHT - LINE_INITIAL - 20, 0, GRAPH_WIDTH-1) < 15 &&
     ...
     GraphProcessingOfLineWhitePointCounting(GRAPH_HIGHT - LINE_INITIAL - 9, 0, GRAPH_WIDTH-1) < 15 &&
     GraphProcessingOfLineWhitePointCounting(GRAPH_HIGHT - LINE_INITIAL - 7, 0, GRAPH_WIDTH-1) < 15 &&
     GraphProcessingOfLineWhitePointCounting(GRAPH_HIGHT - LINE_INITIAL - 6, 0, GRAPH_WIDTH-1) < 15 &&
     GraphProcessingOfLineWhitePointCounting(GRAPH_HIGHT - LINE_INITIAL - 5, 0, GRAPH_WIDTH-1) < 15){
    BEE_ON;
    return false;
  }else{
    BEE_OFF;
